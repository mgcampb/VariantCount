sample.chisq.with.permutation <- function(cl, genelist.rows, geneset_names, chisq.pvals, single.perm.chisq.test) {
  ...
  for(i in 1:M) {
    ...
    sample.perm.dist[i,] <- parSapply(cl, 1:NP, function(j) single.perm.chisq.test(genelist.rows[[i]]))
    #sample.perm.dist[i,] <- sapply(1:NP, function(j) single.perm.chisq.test(genelist.rows[[i]]))
  }
  
  ### Calculate sample.permutation p-values
  cat("\nCalculating sample.permutation p-values\n\tElapsed time =", Sys.time()-t,"\n")
  sample.perm.pvals <- sapply(1:M, function(i) sum(sample.perm.dist[i,] <= chisq.pvals[i])/NP)
